...
import ec.simple.*;
import ec.refactor.RefactorProblem;
import java.io.*;

/* 
 * RefactorEvaluator.java
 * 
...
/**
 * RefactorEvaluator is a simple, non-coevolved generational evaluator which
...
public class RefactorEvaluator extends Evaluator
    ...
    public void evaluatePopulation(final EvolutionState state)
        ...
        if (state.evalthreads==1)
            ...
            for(int y=0;y<state.evalthreads;y++)
                {
                RefactorEvaluatorThread r = new RefactorEvaluatorThread();
                r.threadnum = y;
                r.numinds = numinds[y];
                r.from = from[y];
                r.me = this;
                r.state = state;
                r.p = (RefactorProblem)(p_problem.clone());
                t[y] = new Thread(r);
                t[y].start();
                }
...
class RefactorEvaluatorThread implements Runnable
    {
    public int[] numinds;
    public int[] from;
    public RefactorEvaluator me;
    public EvolutionState state;
    public int threadnum;
    public RefactorProblem p;
